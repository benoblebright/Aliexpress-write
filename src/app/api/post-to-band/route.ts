...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { content, image_url } = requestBody;
    ...
    console.error(`[PROXY-BAND] An unexpected error occurred. Request Body:`, JSON.stringify(requestBody, null, 2));
    console.error('[PROXY-BAND] Error Message:', error.message);
    if (error.response) {
